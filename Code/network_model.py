...
def qsmnet_toy(x, act_func, reuse, isTrain):
    
    conv11 = Conv3d('conv11', x, 2, [5, 5, 5], act_func, reuse, isTrain)
    conv12 = Conv3d('conv12', conv11, 2, [5, 5, 5], act_func, reuse, isTrain)
    pool1 = Max_pool('maxpool1', conv12, [2, 2, 2], reuse)
        
    conv21 = Conv3d('conv21', pool1, 4, [5, 5, 5], act_func, reuse, isTrain)
    conv22 = Conv3d('conv22', conv21, 4, [5, 5, 5], act_func, reuse, isTrain)

    deconv1 = Deconv3d('deconv1', conv22, 2, [2, 2, 2], [2, 2, 2], reuse, isTrain)
    concat1 = Concat('concat1', conv12, deconv1, reuse)
        
    conv31 = Conv3d('conv31', concat1, 2, [5, 5, 5], act_func, reuse, isTrain)
    conv32 = Conv3d('conv32', conv31, 2, [5, 5, 5], act_func, reuse, isTrain)

    out_image = Conv('out_image', conv32, 1, [1, 1, 1], reuse, isTrain)
