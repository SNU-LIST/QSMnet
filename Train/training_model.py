...
def qsmnet_deep(x, keep_prob, reuse, isTrain, ks):
    with tf.variable_scope("qsmnet", reuse=reuse) as scope:
        with tf.variable_scope("conv11", reuse=reuse) as scope:
            conv11 = conv3d(x, [ks, ks, ks, 1, 32], [32], keep_prob, True, isTrain)
            scope.reuse_variables()
        with tf.variable_scope("conv12", reuse=reuse) as scope:
            conv12 = conv3d(conv11, [ks, ks, ks, 32, 32], [32], keep_prob, True, isTrain)
            scope.reuse_variables()
        ...
        with tf.variable_scope("conv21", reuse=reuse) as scope:
            conv21 = conv3d(pool1, [ks, ks, ks, 32, 64], [64], keep_prob, True, isTrain)
            scope.reuse_variables()
        with tf.variable_scope("conv22", reuse=reuse) as scope:
            conv22 = conv3d(conv21, [ks, ks, ks, 64, 64], [64], keep_prob, True, isTrain)
            scope.reuse_variables()
        ...
        with tf.variable_scope("conv31", reuse=reuse) as scope:
            conv31 = conv3d(pool2, [ks, ks, ks, 64, 128], [128], keep_prob, True, isTrain)
            scope.reuse_variables()
        with tf.variable_scope("conv32", reuse=reuse) as scope:
            conv32 = conv3d(conv31, [ks, ks, ks, 128, 128], [128], keep_prob, True, isTrain)
            scope.reuse_variables()
        ...
        with tf.variable_scope("conv41", reuse=reuse) as scope:
            conv41 = conv3d(pool3, [ks, ks, ks, 128, 256], [256], keep_prob, True, isTrain)
            scope.reuse_variables()
        with tf.variable_scope("conv42", reuse=reuse) as scope:
            conv42 = conv3d(conv41, [ks, ks, ks, 256, 256], [256], keep_prob, True, isTrain)
            scope.reuse_variables()
        ...
        with tf.variable_scope("l_conv1", reuse=reuse) as scope:
            l_conv1 = conv3d(pool4, [ks, ks, ks, 256, 512], [512], keep_prob, True, isTrain)
            scope.reuse_variables()
        with tf.variable_scope("l_conv2", reuse=reuse) as scope:
            l_conv2 = conv3d(l_conv1, [ks, ks, ks, 512, 512], [512], keep_prob, True, isTrain)
            scope.reuse_variables()
        ...
        with tf.variable_scope("conv51", reuse=reuse) as scope:
            conv51 = conv3d(deconv_concat4, [ks, ks, ks, 512, 256], [256], keep_prob, True, isTrain)
            scope.reuse_variables()
        with tf.variable_scope("conv52", reuse-reuse) as scope:
            conv52 = conv3d(conv51, [ks, ks, ks, 256, 256], [256], keep_prob, True, isTrain)
            scope.reuse_variables()
        ...
        with tf.variable_scope("conv61", reuse=reuse) as scope:
            conv61 = conv3d(deconv_concat3, [ks, ks, ks, 256, 128], [128], keep_prob, True, isTrain)
            scope.reuse_variables()
        with tf.variable_scope("conv62", reuse=reuse) as scope:
            conv62 = conv3d(conv61, [ks, ks, ks, 128, 128], [128], keep_prob, True, isTrain)
            scope.reuse_variables()
        ...
        with tf.variable_scope("conv71", reuse=reuse) as scope:
            conv71 = conv3d(deconv_concat2, [ks, ks, ks, 128, 64], [64], keep_prob, True, isTrain)
            scope.reuse_variables()
        with tf.variable_scope("conv72", reuse=reuse) as scope:
            conv72 = conv3d(conv71, [ks, ks, ks, 64, 64], [64], keep_prob, True, isTrain)
            scope.reuse_variables()
        ...
        with tf.variable_scope("conv81", reuse=reuse) as scope:
            conv81 = conv3d(deconv_concat1, [ks, ks, ks, 64, 32], [32], keep_prob, True, isTrain)
            scope.reuse_variables()
        with tf.variable_scope("conv82", reuse=reuse) as scope:
            conv82 = conv3d(conv81, [ks, ks, ks, 32, 32], [32], keep_prob, True, isTrain)
            scope.reuse_variables()

        with tf.variable_scope("out", reuse=reuse) as scope:
            out_image = conv(conv82, [1, 1, 1, 32, 1], [1], True)
            scope.reuse_variables()
